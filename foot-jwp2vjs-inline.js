...
const captionElem = $($("#player"+idElem).nextAll("p").contents()[0]).text();
const credElem = $("#player"+idElem).nextAll("p").children("em").text();
const downloadElem = $("#player"+idElem).nextAll("p").children("a").attr('href');
const downloadText = $("#player"+idElem).nextAll("p").children("a").text();
/* Optionals logic */
...
const vidElemHi = $(jwp[0]).text().replace(vidElemHiRegX, `$1`);
...
/* Translator succeeded */
    ...
    /* Show VJS snippet */
    let vjs=`
    <div class="col-xs-10 col-xs-offset-1 log-image">
    \t<video
    \t\tid="video${idElem}"
    ...
    \t</video>
    \t<p class="caption">${captionElem}<em>${credElem}</em> <a href="${downloadElem}" download>${downloadText}</a></p>
    </div>`
    console.log(vjs);
    /* Debug output */
    if(DEBUG) {  
        ...
        console.log("Cred: "+credElem)
        console.log("DownloadHREF: "+downloadElem)
